...
const ProductCard = ({ product }) => {
    ...
    const handleWishList = async (id) => {
        await axios.patch(`https://mini-mart-server.vercel.app/wishlist/add?email=${user?.email}&productID=${id}`,{
            headers:{
    ...
    const handleCart = async(id)=>{
        await axios.patch(`https://mini-mart-server.vercel.app/cart/add?userEmail=${user?.email}&productID=${id}`)
        .then((res) => {
            if (res.data.modifiedCount > 0) {
